...
import com.gezbox.windmap.app.utils.CameraUtils;
import com.gezbox.windmap.app.utils.Constant;
import com.gezbox.windmap.app.utils.NewFFmpegFrameRecorder;
import com.gezbox.windmap.app.widget.ProgressView;
...
public class VideoRecorderActivity extends BaseActivity implements OnClickListener, OnTouchListener {
    private final static String CLASS_LABEL = "VideoRecorderActivity";

    private static final int MESSAGE_ACTION_WARNNING_CHANGE = 2;
    private static final int MESSAGE_ACTION_RECORDING_CHANGE = 3;
    private static final int MESSAGE_ACTION_PROGRESS_CHANGE = 4;
    private static final int MESSAGE_ACTION_RECORDING_SUCESS = 5;
    ...
    Button flashIcon = null,cancelBtn,nextBtn;
    ...
    private int currentResolution = Constant.CameraConstant.RESOLUTION_MEDIUM_VALUE;
    ...
    private void initHandler(){
        mHandler = new Handler(){
            ...
            public void dispatchMessage(Message msg) {
                switch (msg.what) {
                    case MESSAGE_ACTION_WARNNING_CHANGE:
                        int resId = 0;
                        ...
                        break;
                    case MESSAGE_ACTION_RECORDING_CHANGE:
                        if(!recording)
                            initiateRecording();
                        else{
                            //更新暂停的时间
                        ...
                        //setTotalVideoTime();
                        break;
                    case MESSAGE_ACTION_PROGRESS_CHANGE:
                        //设置进度条暂停状态
                        ...
                        if(totalTime >= recordingMinimumTime){
                            ...
                            mHandler.sendEmptyMessage(MESSAGE_ACTION_WARNNING_CHANGE);
                        }else if(totalTime >= recordingChangeTime){
                            currentRecorderState = RecorderState.CHANGE;
                            mHandler.sendEmptyMessage(MESSAGE_ACTION_WARNNING_CHANGE);
                        }
                        break;
                    case MESSAGE_ACTION_RECORDING_SUCESS:
                        currentRecorderState = RecorderState.SUCCESS;
                        mHandler.sendEmptyMessage(MESSAGE_ACTION_WARNNING_CHANGE);
                        break;
                    default:
    ...
    private void initCameraLayout() {
        new AsyncTask<String, Integer, Boolean>(){
            ...
            protected void onPostExecute(Boolean result) {
                ...
                cameraView = new CameraView(VideoRecorderActivity.this, cameraDevice);
                ...
                topLayout.setOnTouchListener(VideoRecorderActivity.this);
    ...
    private void initVideoRecorder() {
        strVideoPath = CameraUtils.createFinalPath(this);

        RecorderParameters recorderParameters = CameraUtils.getRecorderParameter(currentResolution);
    ...
    public class AsyncStopRecording extends AsyncTask<Void,Integer,Void>{
        ...
        protected void onPreExecute() {
            ...
            //创建处理进度条
            creatingProgress= new Dialog(VideoRecorderActivity.this,R.style.Dialog_loading_noDim);
            ...
            creatingProgress.setContentView(R.layout.progress_recorder);
        ...
        private void getFirstCapture(byte[] data){
            ...
            String captureBitmapPath = CameraUtils.createImagePath(VideoRecorderActivity.this);
    ...
    private void showCancellDialog(){
        CameraUtils.showDialog(VideoRecorderActivity.this, "提示", "确定要放弃本视频吗？", 2, new Handler(){
            @Override
            public void dispatchMessage(Message msg) {
                if(msg.what == 1)
                    videoTheEnd(false);
    ...
    class AudioRecordRunnable implements Runnable {
        ...
        private void updateTimestamp()
        {
            if (videoRecorder != null)
            {
                int i = CameraUtils.getTimeStampInNsFromSampleCounted(this.mCount);
            ...
            synchronized (mVideoRecordLock) {
                if (recording && rec && lastSavedframe != null && lastSavedframe.getFrameBytesData() != null && yuvIplImage != null)
                {
                    ...
                    if(nextEnabled && totalTime >= recordingMinimumTime){
                        mHandler.sendEmptyMessage(MESSAGE_ACTION_RECORDING_SUCESS);
                    }
            ...
            if(totalTime< recordingTime){
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        //执行初始化
                        mHandler.removeMessages(MESSAGE_ACTION_RECORDING_CHANGE);
                        mHandler.removeMessages(MESSAGE_ACTION_PROGRESS_CHANGE);
                        mHandler.sendEmptyMessageDelayed(MESSAGE_ACTION_RECORDING_CHANGE,300);
                        break;
                    case MotionEvent.ACTION_UP:
                        mHandler.removeMessages(MESSAGE_ACTION_RECORDING_CHANGE);
                        mHandler.removeMessages(MESSAGE_ACTION_PROGRESS_CHANGE);
                        if(rec)
                            mHandler.sendEmptyMessage(MESSAGE_ACTION_PROGRESS_CHANGE);
    ...
    private void handleSurfaceChanged()
    {
        ...
        //获取摄像头的所有支持的分辨率
        List<Camera.Size> resolutionList = CameraUtils.getResolutionList(mCamera);
        ...
        if(Build.VERSION.SDK_INT >  Build.VERSION_CODES.FROYO)
        {
            mCamera.setDisplayOrientation(CameraUtils.determineDisplayOrientation(VideoRecorderActivity.this, defaultCameraId));
    ...
    {
        Uri videoTable = Uri.parse(Constant.CameraConstant.VIDEO_CONTENT_URI);

        CameraUtils.videoContentValues.put(Video.Media.SIZE, new File(strVideoPath).length());
        try{
            uriVideoPath = getContentResolver().insert(videoTable, CameraUtils.videoContentValues);
        } catch (Throwable e){
            uriVideoPath = null;
            strVideoPath = null;
            e.printStackTrace();
        } finally{}
        CameraUtils.videoContentValues = null;
    }
